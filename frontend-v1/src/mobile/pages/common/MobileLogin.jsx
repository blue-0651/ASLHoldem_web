...
const MobileLogin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const endpoint = userType === 'user' ? '/accounts/token/user/' : '/accounts/token/';
      ...
      setError(err.response?.data?.detail || '로그인에 실패했습니다. 사용자 이름과 비밀번호를 확인하세요.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="mobile-login-container">
      <div className="mobile-logo-container">
        <img src={aslLogo} alt="ASL 홀덤 로고" className="mobile-logo" />
        <h2 style={{ marginTop: '10px', fontSize: '20px', fontWeight: 'bold' }}>ASL 홀덤</h2>
      </div>

      <div className="mobile-login-card">
        <h3 style={{ marginBottom: '20px', fontSize: '18px', textAlign: 'center' }}>로그인</h3>
      ...
      <div style={{ textAlign: 'center', fontSize: '12px', color: '#999', marginTop: '20px' }}>© 2023 ASL 홀덤. 모든 권리 보유.</div>
    </div>
  );
};
